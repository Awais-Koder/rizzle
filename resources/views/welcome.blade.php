...
Route::get('/generate-card-and-download', function () {
    // Process user data and generate card images
    $user = Firebase::database()->getReference('newDb/Users/' . $reference . '/' . $id)->getValue();
    $userName = '';
    $number = '';

    foreach ($user['Card'] as $card) {
        $userName = $card['cardname'];
        $number = $card['cardnumber'];
    }

    // Process front card image
